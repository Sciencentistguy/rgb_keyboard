 ...
 * 2. Call set_*() or load_*() to change the private variables of this class. The setter functions set the setting for \_profile (=1 by default), therefore to
 * change the settings for a different profile call set_profile() first.
...
class rgb_keyboard::keyboard {
 public:
    ...
    enum direction { d_undefined, d_left = 1, d_right = 2, d_up = 1, d_down = 2, d_inwards = 1, d_outwards = 2 };

    /// The variants for the reactive_color led mode
    enum mode_variant { v_undefined, v_color_red, v_color_yellow, v_color_green, v_color_blue };
