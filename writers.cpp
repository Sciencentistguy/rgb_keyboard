...
int rgb_keyboard::keyboard::write_key_mapping_ansi() {
    ...
    for (std::pair<std::string, std::string> element : _keymap[_profile - 1]) {
        ...
        if (_keymap_offsets.find(element.first) != _keymap_offsets.end() && _keymap_options.find(element.second) != _keymap_options.end()) {
            data_remap[_keymap_offsets[element.first][0][0]][_keymap_offsets[element.first][0][1]] = _keymap_options[element.second][0];
            data_remap[_keymap_offsets[element.first][1][0]][_keymap_offsets[element.first][1][1]] = _keymap_options[element.second][1];
            data_remap[_keymap_offsets[element.first][2][0]][_keymap_offsets[element.first][2][1]] = _keymap_options[element.second][2];

            // is key name known and function is a macro?
        } else if (_keymap_offsets.find(element.first) != _keymap_offsets.end() && std::regex_match(element.second, std::regex("macro[0-9]+"))) {
            ...
            if (macronumber <= 100 && macronumber >= 1) {
                data_remap[_keymap_offsets[element.first][0][0]][_keymap_offsets[element.first][0][1]] = 0x05;
                data_remap[_keymap_offsets[element.first][1][0]][_keymap_offsets[element.first][1][1]] = 0x01;
                data_remap[_keymap_offsets[element.first][2][0]][_keymap_offsets[element.first][2][1]] = macronumber - 1;
            }
        }
    }
