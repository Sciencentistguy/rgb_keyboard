...
int rgb_keyboard::keyboard::read_active_profile() {
    ...
    res += libusb_interrupt_transfer(_handle, 0x03, data_read, 64, &transferred, 1000);
...
int rgb_keyboard::keyboard::read_led_settings() {
    ...
    res += libusb_interrupt_transfer(_handle, 0x03, data_read_1, 64, &transferred, 1000);

    // read from endpoint 2
    res += libusb_interrupt_transfer(_handle, 0x82, input_buffer[0], 64, &transferred, 1000);

    // write data packet 2 to endpoint 3
    res += libusb_interrupt_transfer(_handle, 0x03, data_read_2, 64, &transferred, 1000);

    // read from endpoint 2
    res += libusb_interrupt_transfer(_handle, 0x82, input_buffer[1], 64, &transferred, 1000);

    // write data packet 3 to endpoint 3
    res += libusb_interrupt_transfer(_handle, 0x03, data_read_3, 64, &transferred, 1000);

    // read from endpoint 2
    res += libusb_interrupt_transfer(_handle, 0x82, input_buffer[2], 64, &transferred, 1000);
