...
int main(int argc, char** argv) {
    ...
    while ((c = getopt_long(argc, argv, "hc:b:s:d:l:v:P:K:R:M:L:B:D:p:a:kAIr", long_options, &option_index)) != -1) {
        switch (c) {
            case 'h':
                print_help();
                return 0;
    ...
    try {
        ...
        if (flag_bus != flag_device) {  // only -B xor -D: error
            ...
            if (std::regex_match(string_bus, std::regex("[0-9]+")) && std::regex_match(string_device, std::regex("[0-9]+"))) {
        ...
        if (flag_leds) {
            map<string, rgb_keyboard::keyboard::mode> mode_list{{"fixed", rgb_keyboard::keyboard::m_fixed},
        ...
        if (flag_color) {
            ...
            if (string_color == "multi") {  // multicolor
                ...
                kbd.set_color(stoi(string_color.substr(0, 2), 0, 16), stoi(string_color.substr(2, 2), 0, 16), stoi(string_color.substr(4, 2), 0, 16));
                kbd.write_color();
            } else {  // wrong format
